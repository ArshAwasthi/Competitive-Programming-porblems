...
public class Main {
	public static void main(String[] args) {
        ...
        int villages=in.nextInt();
        if(villages%2==0){
            int[] population=new int[villages];
            for(int i=0;i<villages;i++)
                population[i]=in.nextInt();
            for(int i=0;i<villages/2;i++){
                int total_population=population[i]+population[villages-(i+1)];
                System.out.println(total_population/10+" "+(total_population%10));
            }
        }
